    ...
    <ul>
        ...
        <li aid="26">  
            ...
            <c:forEach var = "childmenu" items = "${menu.childmenus }">  
                ...
                <c:out value="${childmenu.menu_name }" />
                </li>
            </c:forEach>
            </ul>  
        </li>
...
$(function() {  
            loadMenu(function (obj){
                ...
                var para_nm = obj.attr('para_nm');
                var para = obj.attr('para');
                if(lastobj){
                    lastobj.attr('style','background:white')
                }
                obj.attr('style','font-weight:bold;');
                lastobj = obj;
                $(window.parent.document).find("#mainFrame").attr("src", "<c:url value='"+contextPath+aid+".do?'/>"+para_nm+"="+para+"&date=Date().getTime()");
